(() => {
  ...
  const SHARED_SHEET = (() => { try { if (typeof CSSStyleSheet !== "function") return null; const sheet = new CSSStyleSheet(); sheet.replaceSync(STYLE); return sheet; } catch (_) { return null; } })();
  ...
  function createUI(row) {
    ...
    root.innerHTML = `${sharedStyles ? "" : `<style>${STYLE}</style>`}<div class="wt-wrap"><div class="wt-bar"><span class="wt-status" role="status" aria-live="polite"></span><button class="wt-action"></button></div><div class="wt-result"></div></div>`;
    const ui = { host, wrap: root.querySelector(".wt-wrap"), status: root.querySelector(".wt-status"), result: root.querySelector(".wt-result"), action: root.querySelector(".wt-action"), jobId: null, canceled: false };
    ...
    ui.action.onclick = () => handleAction(row, ui, messageId);
    ...
    if (job) { ui.jobId = job.jobId; render(ui, job.state, job.text); } else { render(ui, "Pronto"); restore(row, ui); }
    return ui;
  }
  async function handleAction(row, ui, messageId) {
    if (ui.action.dataset.action === "cancel") return cancel(messageId);
  ...
  async function capture(row, ui) {
    ...
    const pending = askPage("capture", {}, 2500);
    if (S.isDownloadButton(button)) button.click();
    try { const response = await pending; if (!response.ok || !response.blob) throw problem("capture_failed", "Áudio indisponível sem reproduzir.", true); return response.blob; }
